...
$(function() {
    ...
    function automaticPostSend(event){
        event.preventDefault();
        ...
        $.ajax({
            url: event.currentTarget.href || this.action,
            type: 'POST',
            dataType: 'json',
            data: data
        }).done(function (response) {
            ...
            if (response.status === 422) {
                Form.addErrors($target, response.responseJSON);
            } else {
    ...
    // Automatic post
    $('*:not(form).automatic-post').on('click', automaticPostSend);
    $('form.automatic-post').on('submit', automaticPostSend);
